...
def fixFileNames(sender, instance, created, **kwargs):
    '''
    Endre opplastede filnavn til pk dersom instansen opprettes samtidig som fila "None.mp3". 
    ...
    for field in sender._meta.get_fields():
        if isinstance(field, FileField):
            ...
            newName = str(instance.pk) + ('' if '.' not in fil.name else '.' + fil.name.split('.')[-1])
            if os.path.split(fil.name)[1] != newName:
                newPath = os.path.join(os.path.split(fil.path)[0], newName)

                # Endre navn på sjølve fila
                os.rename(fil.path, newPath)

                # Endre pathen til fila i modellen
                fil.name = os.path.join(os.path.split(fil.name)[0], newName)
