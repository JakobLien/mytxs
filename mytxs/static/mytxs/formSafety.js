        ...
        if(change.initialValue.constructor.name === "Array"){
            for(const option of change.element.options){
                if(change.value.includes(option.text) && !change.initialValue.includes(option.text)){
                    changeText.push(`${change.labelText} la til "${option.text}"`);
                }else if(!change.value.includes(option.text) && change.initialValue.includes(option.text)){
                    changeText.push(`${change.labelText} fjernet "${option.text}"`);
                }
            }
        }else{
            changeText.push(`${change.labelText} endret fra "${change.initialValue}" til "${change.value}"`);
        }
    }
    return changeText
}
...
function evaluateManagedForm(form){
    ...
    // Sett inn tekst som beskriver endringen:
    form.querySelector(".changes").innerHTML = "<p>"+formatFormChanges(changes).join("<br>")+"</p>"
